...
class StaticJSTest(TestCase):
    ...
    def test_staticjs_defaults(self):
        """Test that staticjs settings have the correct defaults"""
        self.assertEqual(
            self.storage.staticjs_exclude_patterns,
            ["*.js", "*.css", "*.scss", "*.sass", "*.less", "*.ts", "*.tsx"],
        )
    ...
    def test_staticjs_manifest_creation(self):
        ...
        results = self.storage._create_staticjs_manifest()
    ...
    def test_filtered_static_paths(self):
        ...
        # Get filtered paths
        filtered_dict = self.storage._get_filtered_static_paths()
    ...
    def test_custom_exclude_patterns(self):
        """Test that custom exclude patterns work correctly"""
        # Create storage with custom exclude patterns
        storage = EnhancedManifestStaticFilesStorage(
            staticjs_exclude_patterns=["*.json"]
        )
        ...
        # Get filtered paths
        filtered_dict = storage._get_filtered_static_paths()

        # Check that only JSON files are excluded
        self.assertIn("image.png", filtered_dict)
        self.assertNotIn("data.json", filtered_dict)
        self.assertIn("script.js", filtered_dict)
